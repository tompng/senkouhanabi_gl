...
void main() {
  ...
  if (z > 0.0) {
    ...
    vec2 b = ballRFunc(t);
    float delta = 0.01;
    vec2 db = (b - ballRFunc(t-delta)) / delta;
    float dr = db.x / db.y;
  ...
  vPosition = vec3(0.0*c + r * xy, z);// + windMove * exp(-4.0 * z);
...
void main() {
  ...
  gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(2.0,1.0,0.5), ttt);
}
`
